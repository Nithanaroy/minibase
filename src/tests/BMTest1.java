...
class BMDriver extends TestDriver implements GlobalConst {
	...
	protected boolean test2() {
		...
		boolean status = OK;
		for (int i = 0; i < 3; i++) {
			System.out.print("\n  Test 2 exercises some illegal buffer " + "manager operations:" + replacealgo[i] + "\n");
			...
			if (status == OK && SystemDefs.JavabaseBM.getNumUnpinnedBuffers() != 0) {
				status = FAIL;
				System.err.print("*** The buffer manager thinks it has " + SystemDefs.JavabaseBM.getNumUnpinnedBuffers()
	...
	protected boolean test3() {
		...
		boolean status = OK;
		for (int i = 0; i < 3; i++) {
			System.out.print("\n  Test 3 exercises some of the internals " + "of the buffer manager" + replacealgo[i] + "\n");
...
//		boolean status = OK;
//		SystemDefs sysdef = new SystemDefs(dbpath, NUMBUF + 20, NUMBUF, "LRUK");
//		LRU replacer = (LRU) (SystemDefs.JavabaseBM.getReplacer());
...
//			int pagenumber = -1;
//
//			for (int index = 0; status == OK && index < numPages; ++index) {
//				try {
//					pagenumber = frames[index];
